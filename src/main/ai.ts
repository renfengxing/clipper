...
export async function autoTagClips(
  ...
  const tagRule =
    videoTags.length > 0
      ? `只能从以下标签中选择（可多选，也可不选）：${videoTags.join('、')}`
      : `请自行提出简洁标签（每个不超过15字，如：进球、过人、失误、防守等），同类尽量复用同一标签`
  const prompt = `你是青少年足球比赛视频片段的标签助手。下面是按编号列出的片段标题，请给每个片段打标签。
${tagRule}

片段列表：
${list}

只返回一个 JSON 数组，长度与片段数相同，每个元素是该片段的标签字符串数组（无合适标签则空数组）。例如：[["进球"],["过人","失误"],[]]
不要任何解释或多余文字。`
